...
my $filename = 'memory_profile.log';
my $filename2 = 'profile.log';
my $fileMem;
my $fileCpu;
...
if (defined $profile) {
#open($fileMem, '>', $filename) or die "Could not open file '$filename' $!"; 
open($fileCpu, '>', $filename2) or die "Could not open file '$filename2' $!"; }
#### 
...
#### Modified ####
if (defined $profile) { #close $fileMem;
	 close $fileCpu; }
...
sub foreach_applicable_process
{
	...
	if ($watchfor eq 'tree') {
		...
		# 26944 12496 kmail
		my $chars = "\t \\_|";
		my $PS_FH; open $PS_FH, "-|", qw(ps -e f -o pgrp= -o pid= -o vsz= -o %cpu= -o ucmd=) or die "Bad open ps: $!";
		...
		while (<$PS_FH>){
			...
			my ($grp,$pid,$vsz,$depth_str,$cmd,$ucpu) = ($1,$2,$3,$5,$6,$4);
			if ($state == 0){
			...
			#print "Print list ($1,$2,$3,$4,$5,$6) \n"; 
			$sub->($pid,$grp,$cmd,$vsz,$ucpu);
		}
		...
		my $PS_FH; open $PS_FH, "-|", qw(ps -A -o pgrp= -o pid= -o vsz= -o ucmd= -o %cpu=) or die "Bad open ps: $!";
		while (<$PS_FH>){
			/^\s*([0-9]+)\s*([0-9]+)\s*([0-9]+)\s*(.*)\s*([.0-9]+)/ or next;
			print "Print other list ($1,$2,$3,$4) \n"; 
			my ($grp,$pid,$vsz,$cmd,$ucpu) = ($1,$2,$3,$4);
			$grp == $pgrp or next;
...
sub update_memory
{
	...
	my ($pgrp) = @_;
	my $result = 0;
	foreach_applicable_process($pgrp,$watchfor,sub { my ($pid,$grp,$cmd,$vsz,$ucpu) = @_;
		$result += $vsz;
	});
	return $result;
}
...
sub update_cpu
{
	# Calculate the amount of cpu consumed by the process group given
	my ($pgrp) = @_;
	my $result = 0;
	foreach_applicable_process($pgrp,$watchfor,sub { my ($pid,$grp,$cmd,$vsz,$cpu) = @_;
		$result += $cpu;
	});
	#print "Rusults $result\n";
	return $result;
}
...
sub limits_exceeded
{
	if (defined $profile) {	print $fileCpu $meminfo," ",update_cpu($blackbox_pid,$meminfo),"\n";}
